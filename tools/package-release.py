...
manifest={'package':app.get('package','chat.oldy'),'version_code':int(app.get(android+'versionCode')),'version_name':app.get(android+'versionName'),'size':apk.stat().st_size,'sha256':hashlib.sha256(apk.read_bytes()).hexdigest(),'notes':'Oldi 0.6.0: компактные настройки и нижнее меню, YouTube внутри чата, локальная зашифрованная история видео и продолжение просмотра, локальный сетевой режим без видеотрафика на сервере Oldi, авторские стикеры из фото, обработка щелчков и выбранных фрагментов аудио. Продажи стикеров требуют подключения платежей; сейчас доступны личные черновики предложений. Аккаунт и переписка сохраняются.'}
(release/'release.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2))
with tarfile.open(build/'OldyChat-0.6.0-server.tar.gz','w:gz') as t:
 for f in sorted(release.iterdir()):t.add(f,arcname=f.name)
print(json.dumps({'archive':str(build/'OldyChat-0.6.0-server.tar.gz'),'apk_sha256':manifest['sha256']}))
