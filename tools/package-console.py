...
PUBLISHER
archive_name='OldyChat-0.6.2-server.tar.gz'
...
with tempfile.TemporaryDirectory(prefix='oldy-update-') as tmp:
 ...
 if bundle is None:
  print('Скачиваем OldЫ Chat 0.6.2…',flush=True);downloaded=folder/'release.zip'
 ...
 if manifest['package']!='chat.oldy' or manifest['version_code']!=11 or manifest['sha256']!=hashlib.sha256(apk.read_bytes()).hexdigest() or manifest['size']!=apk.stat().st_size:raise SystemExit('Проверка APK не пройдена. Сервер не изменён.')
 ...
 print('OLDY CHAT: UPDATE AVAILABLE 0.6.2-beta',flush=True)
...
ast.parse(script)
args.output.write_text("python3 - <<'OLDY_UPDATE_062'\n"+script+"OLDY_UPDATE_062\n")
