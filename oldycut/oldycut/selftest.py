...
def run(folder):
    ...
    app=QApplication.instance() or QApplication([]);app.setStyle('Fusion');app.setStyleSheet(STYLE);w=MainWindow(restore=False);w.project=p;w.refresh();w.show();app.processEvents();w.grab().save(str(folder/'app.png'))
    from PySide6.QtTest import QTest
    def timed_video(r,progress):
        return r.run(['-re','-f','lavfi','-i','testsrc2=s=160x90:r=20:d=2.2','-f','null','-'],duration=2.2,label='Проверка обработки видео')
    w.start_job(timed_video,lambda result:None,'Проверка сборки · реальное видео',True)
    capture_deadline=time.monotonic()+5
    while w.worker and time.monotonic()<capture_deadline:
        app.processEvents();time.sleep(.025)
        if w.job_dialog.bar.maximum()==100 and w.job_dialog.bar.value()>0:break
    w.job_dialog.grab().save(str(folder/'progress.png'))
    deadline=time.monotonic()+15
    while w.worker and time.monotonic()<deadline:QTest.qWait(30)
    assert w.worker is None and w.job_dialog.bar.value()==100,'Progress operation did not finish'
    w.close()
    report={'platform':platform.platform(),'architecture':platform.machine(),'ffmpeg':Path(ffmpeg_path()).name,'render_size':out.stat().st_size,'duration':check.duration,'qt_ui':'ok','progress_ui':'ok','api':'not called'}
