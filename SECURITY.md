...
No application administrator can decrypt message content merely by possessing the server account database. The prototype pins contacts' public keys and exposes fingerprints for verification, but first use still depends on the directory server (TOFU). Account passwords alone are not enough to recover private message keys. New devices need an encrypted user backup.
