...
final class WelcomeArt extends View {
 ...
 WelcomeArt(Context context){super(context);portrait=BitmapFactory.decodeResource(getResources(),R.drawable.launcher_art);setContentDescription("Старый геймер в кепке переписывается на телефоне");}
 protected void onDraw(Canvas c){super.onDraw(c);float t=motion?(SystemClock.uptimeMillis()-began)/1000f:1.4f,w=getWidth(),h=getHeight(),size=Math.min(w,h)*.84f,left=(w-size)/2,top=(h-size)/2;float bob=motion?(float)Math.sin(t*4)*h*.008f:0;
  ...
  float enter=Math.min(1,Math.max(0,(t-.15f)*2));bubble(c,w*.04f,h*.22f+(1-enter)*14,w*.32f,h*.14f,"Привет!",enter,0xffd5edff,0xff203e55);
