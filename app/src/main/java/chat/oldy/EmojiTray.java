...
final class EmojiTray {
 static final String[] NAMES={"Смайлы","Жесты","Любовь","Игры","Еда","Живые","Стикеры"};
 ...
 static void fill(MainActivity a,int category){if(a.emojiPanel==null)return;a.emojiPanel.removeAllViews();a.emojiPanel.setBackground(a.shape(a.CARD,18));
  ...
  if(category==6){TextView mine=a.button(I18n.t("＋ Мои стикеры · создать из фото"),false,()->PersonalStickers.show(a));a.emojiPanel.addView(mine,new LinearLayout.LayoutParams(-1,a.dp(42)));}
  ...
  String[] values=category<5?FACES[category].split(" "):category==5?MotionEmoji.GLYPHS:GamerSticker.CAPTIONS;int cellHeight=category==6?82:52;
  for(int i=0;i<values.length;i++){final int n=i;View cell;if(category==5)cell=new MotionEmoji(a,i);else if(category==6)cell=new GamerSticker(a,i);else{TextView t=a.label(values[i],27,a.TEXT);t.setGravity(Gravity.CENTER);cell=t;}
   ...
   cell.setOnClickListener(v->{if(category==6){try{a.send(new JSONObject().put("kind","sticker").put("sticker",100+n).put("text",I18n.t(GamerSticker.CAPTIONS[n])),null);}catch(Exception e){a.error(Api.message(e));}return;}int at=Math.max(0,a.compose.getSelectionStart());a.compose.getText().insert(at,values[n]);if(category==5)a.selectedMotion=n;});
  }
