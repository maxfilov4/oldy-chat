...
final class WallpaperPicker {
 static void show(MainActivity a){
  ...
  a.paragraph(body,I18n.t("Тема применяется ко всему Oldi: чатам, меню, контактам и настройкам."));a.space(body,12);
  ...
  for(int row=0;row<(Wallpaper.NAMES.length+1)/2;row++){
   LinearLayout pair=a.row();for(int column=0;column<2;column++){
    final int style=row*2+column;if(style>=Wallpaper.NAMES.length)continue;boolean selected=!photo&&Notices.prefs(a).getInt("wallpaper",a.cyber?3:0)==style;
