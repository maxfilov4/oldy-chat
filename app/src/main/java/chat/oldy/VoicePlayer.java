...
final class VoicePlayer {
 ...
 void toggle(JSONObject message){String next=message.optString("id");
  ...
  a.work.execute(()->{try{JSONObject m=account.message(next);if(m==null)throw new IOException("Сообщение удалено");
    if(!m.has("local")||!MediaFiles.path(a,m.optString("local")).isFile()){
     if(m.optString("cloud_blob").isEmpty())throw new IOException("Запись недоступна. Открой вложение через меню сообщения.");
     CloudMedia.download(a,account,m);m=account.message(next);
    }
    final byte[] bytes=MediaFiles.bytes(a,m.getString("local"));if(bytes.length<100)throw new IOException("Запись не содержит аудио");
    a.runOnUiThread(()->{if(serial!=generation||account!=a.vault)return;try{
      ...
      current.setOnErrorListener((p,w,e)->{if(serial==generation)fail("Не удалось воспроизвести запись. Нажми, чтобы повторить.");return true;});
      current.prepareAsync();
     }catch(Exception e){fail("Не удалось открыть голосовое сообщение");}});
