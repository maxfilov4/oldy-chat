...
final class VoiceNoteView extends LinearLayout {
 ...
 VoiceNoteView(MainActivity a,JSONObject message){super(a);this.a=a;this.message=message;mid=message.optString("id");setOrientation(HORIZONTAL);setGravity(Gravity.CENTER_VERTICAL);setMinimumWidth(a.dp(215));setPadding(0,a.dp(4),0,a.dp(4));setContentDescription(I18n.t("Голосовое сообщение"));
  play=new UiIcon(a,"play",a.light?0xff146c9c:0xff98e5ff);play.setBackground(a.shape(a.light?0xffc9e7fb:0xff194b66,18));play.setContentDescription(I18n.t("Воспроизвести голосовое сообщение"));play.setOnClickListener(v->{if(a.voicePlayer==null)a.voicePlayer=new VoicePlayer(a);a.voicePlayer.toggle(message);refresh();});addView(play,new LayoutParams(a.dp(44),a.dp(44)));
  LinearLayout info=a.col();info.setPadding(a.dp(11),0,0,0);timeline=new Timeline(a,message.optString("waveform"));info.addView(timeline,new LayoutParams(-1,a.dp(32)));time=a.label(I18n.t("Голосовое сообщение"),11,a.MUTED);info.addView(time,new LayoutParams(-1,-2));addView(info,new LayoutParams(0,-2,1));
 ...
 void refresh(){VoicePlayer p=a.voicePlayer;boolean current=p!=null&&mid.equals(p.id);int duration=current?p.duration(mid):0;if(duration==0)duration=message.optInt("duration");int position=current?p.position(mid):0;
  boolean playing=p!=null&&p.playing(mid);play.symbol(playing?"pause":"play");play.setContentDescription(playing?I18n.t("Пауза голосового сообщения"):I18n.t("Воспроизвести голосовое сообщение"));
  time.setText(current&&p.loading?I18n.t("Загружаем аудио…"):duration>0?(position>0?clock(position)+" / ":"")+clock(duration):I18n.t("Голосовое · ")+a.size(message.optLong("size")));
 ...
 static final class Timeline extends View{
  ...
  Timeline(MainActivity a,String encoded){super(a);light=a.light;try{values=Crypto.un64(encoded);if(values.length>96)values=new byte[0];}catch(Exception ignored){values=new byte[0];}setContentDescription(I18n.t("Прогресс голосового сообщения"));}
