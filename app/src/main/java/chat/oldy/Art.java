...
final class Art extends View {
 static Bitmap mascot;static final android.util.LruCache<String,Bitmap> photos=new android.util.LruCache<>(40);
 ...
 Art(Context c,String k){super(c);key=k==null?"preset:0":k;type=key.equals("mascot")?2:key.startsWith("sticker:")?1:0;int n=0;try{n=Integer.parseInt(key.substring(key.indexOf(':')+1));}catch(Exception ignored){}index=Math.max(0,Math.min(11,n));setContentDescription(type==2?"Дедушка со смартфоном":type==1?"Анимированный эмодзи":"Аватар");
  if(type==2&&mascot==null)try{BitmapFactory.Options opt=new BitmapFactory.Options();opt.inSampleSize=2;mascot=BitmapFactory.decodeStream(c.getAssets().open("mascot.png"),null,opt);}catch(Exception ignored){}
 }
 ...
 protected void onDraw(Canvas c){super.onDraw(c);float w=getWidth(),h=getHeight();boolean moving=Notices.prefs(getContext()).getBoolean("animations",true);float t=moving?(System.currentTimeMillis()%100000)/1000f:0;
  ...
  if(type==2&&mascot!=null){
   int grid=12;float[] mesh=new float[(grid+1)*(grid+1)*2];int at=0;float sway=(float)Math.sin(t*7)*w*.003f;
   for(int y=0;y<=grid;y++)for(int x=0;x<=grid;x++){float xx=x*w/grid,yy=y*h/grid;float dx=xx-w*.56f,dy=yy-h*.36f;float f=(float)Math.exp(-(dx*dx/(w*w*.03f)+dy*dy/(h*h*.01f)));mesh[at++]=xx+sway*f;mesh[at++]=yy+sway*f;}
   c.drawBitmapMesh(mascot,grid,grid,mesh,0,null,0,p);
   for(int i=0;i<3;i++)oval(c,0xffc0f882,w*(.72f+i*.05f),h*.30f+(float)Math.sin(t*5-i)*h*.006f,w*(.742f+i*.05f),h*.322f+(float)Math.sin(t*5-i)*h*.006f);
