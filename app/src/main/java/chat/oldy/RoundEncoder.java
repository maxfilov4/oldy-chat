...
final class RoundEncoder {
 ...
 synchronized void packet(boolean v,ByteBuffer buffer,MediaCodec.BufferInfo info)throws IOException{if(info.size==0||(info.flags&MediaCodec.BUFFER_FLAG_CODEC_CONFIG)!=0)return;byte[] bytes=new byte[info.size];buffer.position(info.offset);buffer.limit(info.offset+info.size);buffer.get(bytes);Packet p=new Packet(v,bytes,Math.max(0,info.presentationTimeUs),info.flags);if(muxStarted)write(p);else{pendingBytes+=bytes.length;if(pendingBytes>4*1024*1024)throw new IOException(I18n.t("Не удалось начать запись звука"));pending.add(p);}}
 ...
 void finish()throws Exception{
  ...
  if(!complete||failure!=null||videoSamples<8||audioSamples<4||file.length()<1000){file.delete();throw new IOException(I18n.t("Не удалось сохранить кружок. Попробуй записать ещё раз."),failure);}
 }
 void cleanup(){
