...
public class VideoActivity extends Activity {
 ...
 public void onCreate(Bundle b){
  ...
  root=new FrameLayout(this);root.setBackgroundColor(Color.BLACK);surface=new TextureView(this);root.addView(surface,new FrameLayout.LayoutParams(-1,-1,Gravity.CENTER));surface.setOnClickListener(v->setControls(!uiVisible));
  ...
  root.setOnApplyWindowInsetsListener((v,insets)->{if(Build.VERSION.SDK_INT>=28&&insets.getDisplayCutout()!=null){android.view.DisplayCutout c=insets.getDisplayCutout();root.setPadding(c.getSafeInsetLeft(),c.getSafeInsetTop(),c.getSafeInsetRight(),c.getSafeInsetBottom());}return insets;});
 ...
 void prepareQuality(int height){if(video==null||destroyed)return;int generation=++conversionGeneration;requestedQuality=height;converting=true;conversionMessage=I18n.t("Готовим видео · текущий ролик можно смотреть");setControls(true);pollConversion(generation,height);}
 void pollConversion(int generation,int height){if(destroyed||generation!=conversionGeneration)return;io.execute(()->{try{JSONObject data=new JSONObject().put("id",video);if(generation==conversionGeneration)data.put("retry",true);if(height>0)data.put("quality",height);JSONObject r=new Api(this).call(height>0?"/videos/quality":"/videos/compatible",data,ChatService.vault(this).token());String state=r.optString("state");handler.post(()->{if(destroyed||generation!=conversionGeneration)return;if(state.equals("ready")){converting=false;conversionMessage="";quality=height;compatible=height==0;startPlayer(false);loadQualities();}else if(state.equals("error")){converting=false;conversionMessage=I18n.t("Не удалось подготовить качество. Оригинал доступен.");}else{conversionMessage=I18n.t("Подготовка качества · можно продолжать просмотр");handler.postDelayed(()->pollConversion(generation,height),4000);}});}catch(Exception e){handler.post(()->{if(generation!=conversionGeneration||destroyed)return;converting=false;conversionMessage=Api.message(e);});}});}
 void fit(){root.post(()->{if(vw<1||vh<1)return;int w=root.getWidth()-root.getPaddingLeft()-root.getPaddingRight(),h=root.getHeight()-root.getPaddingTop()-root.getPaddingBottom();float ratio=vw*Math.max(.1f,pixelRatio)/vh;if(w>h*ratio)w=Math.round(h*ratio);else h=Math.round(w/ratio);surface.setLayoutParams(new FrameLayout.LayoutParams(w,h,Gravity.CENTER));});}
