...
public class ChatService extends Service {
 ...
 void sync()throws Exception{
  try{api.call("/capabilities",new JSONObject().put("protocol",4),vault.token());long revision=vault.roomRevision();vault.syncRooms(api.call("/rooms",null,vault.token()).getJSONArray("rooms"),revision);vault.profile(api.call("/me",null,vault.token()));vault.blocks(api.call("/blocks",null,vault.token()).getJSONArray("blocked"));deletionSync();cloudSync();}catch(Api.Failure e){if(e.status!=404)throw e;}
 }
 void send(){long lastSync=0;while(running){try{
   ...
   for(int i=0;i<a.length()&&running;i++){JSONObject m=a.getJSONObject(i);if(!vault.has(m.optString("id")))continue;if(m.optBoolean("out")&&!m.optBoolean("registered")&&!m.optString("kind").equals("control")){try{register(api,vault,m);vault.markRegistered(m.getString("id"));}catch(Api.Failure e){if(e.status==410){deletionSync();continue;}if(e.status!=404)continue;}}if(!m.optBoolean("cloud")&&!m.optString("status").equals("pending")&&archived++<10){try{api.call("/history/store",new JSONObject().put("envelope",vault.archiveEnvelope(m)),vault.token());vault.cloudSaved(m.getString("id"));}catch(Exception ignored){}}if(!m.optString("status").equals("pending"))continue;
