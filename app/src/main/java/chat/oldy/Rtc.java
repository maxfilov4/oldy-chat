...
final class Rtc {
 ...
 void request(JSONObject m){io.execute(()->{try{String mid=m.getString("id"),from=m.getString("from");if(m.has("local"))return;wanted.put(mid,from);status.put(mid,I18n.t("Соединяем телефоны…"));signal.send(from,new JSONObject().put("op","request").put("mid",mid).put("room",Conversation.community(m.optString("peer"))?Conversation.room(m.optString("peer")):""));io.schedule(()->{if(wanted.remove(mid)!=null){status.put(mid,I18n.t("Нет прямого соединения. Попробуйте Wi-Fi и откройте чат на обоих телефонах."));ChatService.changed(context);}},65,TimeUnit.SECONDS);}catch(Exception e){status.put(m.optString("id"),Api.message(e));}ChatService.changed(context);});}
 ...
 void close(){io.execute(()->{for(Session s:new ArrayList<>(sessions.values()))s.finish(false,I18n.t("Передача остановлена"));factory.dispose();});io.shutdown();}
 class Session {
  ...
  void connect(){
   ...
   pc=factory.createPeerConnection(config,new PeerConnection.Observer(){
    ...
    public void onIceConnectionChange(PeerConnection.IceConnectionState s){if(s==PeerConnection.IceConnectionState.FAILED)io.execute(()->finish(false,I18n.t("Сеть не разрешила прямую передачу. Попробуйте Wi-Fi.")));}
   ...
   if(pc==null)throw new IllegalStateException(I18n.t("Не удалось соединить телефоны"));
   io.schedule(()->{if(!ended&&dc!=null&&dc.state()!=DataChannel.State.OPEN)finish(false,I18n.t("Нет прямого соединения. Откройте чат на обоих телефонах."));},55,TimeUnit.SECONDS);
   io.schedule(()->{if(!ended)finish(false,I18n.t("Передача прервана. Нажмите ещё раз для повтора."));},8,TimeUnit.MINUTES);
  }
  SdpObserver creator(String type){return new SdpObserver(){
   public void onCreateSuccess(SessionDescription s){io.execute(()->{if(ended)return;sdpType=type;pc.setLocalDescription(new SdpObserver(){public void onSetSuccess(){io.schedule(()->sendSdp(),8,TimeUnit.SECONDS);}public void onSetFailure(String x){io.execute(()->finish(false,I18n.t("Не удалось соединить телефоны")));}public void onCreateSuccess(SessionDescription s){}public void onCreateFailure(String s){}},s);});}
   public void onCreateFailure(String s){io.execute(()->finish(false,I18n.t("Не удалось соединить телефоны")));}public void onSetSuccess(){}public void onSetFailure(String s){}
  };}
  void sendSdp(){if(ended||sentSdp||sdpType==null||pc.getLocalDescription()==null)return;try{String s=pc.getLocalDescription().description;if(s.length()>14000)throw new IOException();signal.send(peer,new JSONObject().put("op",sdpType).put("mid",mid).put("sid",sid).put("sdp",s).put("room",Conversation.community(m.optString("peer"))?Conversation.room(m.optString("peer")):""));sentSdp=true;}catch(Exception e){finish(false,I18n.t("Не удалось отправить запрос передачи"));}}
  void remote(String type,String sdp){if(sdp.length()>14000)return;pc.setRemoteDescription(new SdpObserver(){public void onSetSuccess(){if(type.equals("offer"))io.execute(()->pc.createAnswer(creator("answer"),new MediaConstraints()));}public void onSetFailure(String s){io.execute(()->finish(false,I18n.t("Не удалось соединить телефоны")));}public void onCreateSuccess(SessionDescription s){}public void onCreateFailure(String s){}},new SessionDescription(type.equals("offer")?SessionDescription.Type.OFFER:SessionDescription.Type.ANSWER,sdp));}
  void channel(DataChannel d){if(ended){d.close();return;}dc=d;dc.registerObserver(new DataChannel.Observer(){
   public void onBufferedAmountChange(long amount){}
   public void onStateChange(){io.execute(()->{if(ended)return;if(dc.state()==DataChannel.State.OPEN){wanted.remove(mid);status.put(mid,I18n.t("Передача…"));ChatService.changed(context);if(sending)startSend();}else if(dc.state()==DataChannel.State.CLOSED)finish(false,I18n.t("Передача прервана. Нажмите для повтора."));});}
  ...
  void startSend(){if(input!=null||ended)return;try{input=MediaFiles.open(context,m.getString("local"));pump();}catch(Exception e){finish(false,I18n.t("Не удалось открыть вложение"));}}
  void pump(){if(ended||dc.state()!=DataChannel.State.OPEN)return;try{
   ...
   status.put(mid,I18n.t("Отправлено ")+(received*100/Math.max(1,m.optLong("size")))+"%");io.schedule(()->pump(),25,TimeUnit.MILLISECONDS);
  }catch(Exception e){finish(false,I18n.t("Передача прервана. Нажмите для повтора."));}}
  void consume(byte[] bytes,boolean binary){if(ended)return;try{
   if(sending){if(!binary&&new String(bytes,"UTF-8").equals("saved")){finish(true,I18n.t("Передано"));}return;}
   if(output==null)output=new MediaFiles.Writer(context,mid);
   if(binary){if(received+bytes.length>m.getLong("size"))throw new IOException();output.put(bytes);received+=bytes.length;status.put(mid,I18n.t("Загрузка ")+(received*100/m.getLong("size"))+"%");ChatService.changed(context);}
    ...
    vault.attachment(mid,mid);if(!dc.send(new DataChannel.Buffer(ByteBuffer.wrap(Crypto.bytes("saved")),false)))throw new IOException();status.put(mid,I18n.t("Готово"));ChatService.changed(context);io.schedule(()->finish(true,I18n.t("Готово")),2,TimeUnit.SECONDS);
   }
  }catch(Exception e){finish(false,I18n.t("Не удалось получить файл. Нажмите для повтора."));}}
