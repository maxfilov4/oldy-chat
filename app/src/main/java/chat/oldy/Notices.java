...
final class Notices {
 ...
 static void show(Context c,Vault v,JSONObject m,JSONObject user){
  ...
  NotificationManager nm=c.getSystemService(NotificationManager.class);NotificationChannel channel=new NotificationChannel(ch,I18n.t("Сообщения")+(rule.sound?" · "+I18n.t(TONE_NAMES[tone(c)]):I18n.t(" · без звука")),NotificationManager.IMPORTANCE_HIGH);
  ...
  Notification n=new Notification.Builder(c,ch).setSmallIcon(R.drawable.ic_launcher).setContentTitle(user.optString("name",I18n.t("OldЫ Chat"))).setContentText(preview?Payload.preview(m):I18n.t("Новое сообщение")).setContentIntent(p).setAutoCancel(true).setCategory(Notification.CATEGORY_MESSAGE).setExtras(extras).setVisibility(Notification.VISIBILITY_PRIVATE).setPublicVersion(new Notification.Builder(c,ch).setSmallIcon(R.drawable.ic_launcher).setContentTitle(I18n.t("OldЫ Chat")).setContentText(I18n.t("Новое сообщение")).build()).build();try{nm.notify(peer.hashCode(),n);}catch(SecurityException ignored){}
 }
}
