...
final class YouTubeDomainRules {
 static final Set<String> ROOTS=Collections.unmodifiableSet(new HashSet<>(Arrays.asList("youtube.com","youtube-nocookie.com","youtu.be","googlevideo.com","ytimg.com","youtubei.googleapis.com","youtube.googleapis.com","ggpht.com")));
 ...
 static YouTubeDomainRules load(Context c)throws Exception{java.io.File saved=new java.io.File(c.getFilesDir(),"youtube-rules.json");if(saved.isFile())try(java.io.InputStream in=new java.io.FileInputStream(saved)){return new YouTubeDomainRules(new JSONObject(Api.read(in,8192)));}catch(Exception invalid){}try(java.io.InputStream in=c.getAssets().open("youtube-network.json")){return new YouTubeDomainRules(new JSONObject(Api.read(in,8192)));}}
