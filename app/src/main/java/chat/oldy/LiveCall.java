...
final class LiveCall {
 ...
 static synchronized void dial(Context c,String peer){
  ...
  try{Vault v=ChatService.vault(c);if(peer.equals(v.nick())||!peer.matches("[a-z0-9_]{3,24}"))return;
   if(v.isBlocked(peer))throw new Exception("Собеседник заблокирован");
 ...
 static synchronized void receive(ChatService service,String peer,JSONObject payload){
  ...
  if(op.equals("call_offer")){
   ...
   Session ringing=s;s.timer.schedule(()->{if(!ringing.accepted)ringing.end("Пропущенный звонок",true);},45,TimeUnit.SECONDS);
   ...
   if(op.equals("call_end")||op.equals("call_busy"))call.end(op.equals("call_busy")?"Собеседник занят":"Звонок завершён",false);
  }
 }
 ...
 static final class Session {
  ...
  Session(Context context,Vault v,String p,String id,boolean in,String sdp){c=context.getApplicationContext();vault=v;peer=p;sid=id;incoming=in;offer=sdp;state=in?"Входящий звонок":"Вызываем…";}
  ...
  synchronized void start(){
   ...
   try{ChatService service=ChatService.instance;if(service==null||!service.running||service.vault!=vault)throw new Exception("Откройте чат и повторите звонок");
    service.callForeground(true,peer);accepted=true;c.getSystemService(NotificationManager.class).cancel(410);
   }catch(Exception e){end("Не удалось включить микрофон для звонка",true);return;}
   execute(()->{try{
    ...
    pc=factory.createPeerConnection(rtc,new PeerConnection.Observer(){
     ...
     public void onIceConnectionChange(PeerConnection.IceConnectionState s){
      if(ended)return;if(s==PeerConnection.IceConnectionState.CONNECTED||s==PeerConnection.IceConnectionState.COMPLETED){if(!connected){connectedAt=SystemClock.elapsedRealtime();connected=true;}state="На связи";ChatService.changed(c);}
      if(s==PeerConnection.IceConnectionState.DISCONNECTED){state="Восстанавливаем соединение…";try{timer.schedule(()->{if(!ended&&state.equals("Восстанавливаем соединение…"))end("Связь прервалась",true);},20,TimeUnit.SECONDS);}catch(RejectedExecutionException ignored){}}
      if(s==PeerConnection.IceConnectionState.FAILED)end("Не удалось соединить звонок",true);
     }
    ...
    if(pc==null)throw new Exception("Не удалось включить звонок");audioSource=factory.createAudioSource(new MediaConstraints());audio=factory.createAudioTrack("oldy-mic",audioSource);audio.setEnabled(!muted);pc.addTrack(audio,Collections.singletonList("oldy-call"));
    ...
    timer.schedule(()->{if(!connected)end("Нет ответа",true);},65,TimeUnit.SECONDS);
  ...
  SdpObserver description(){return new SdpObserver(){
   ...
   public void onSetSuccess(){}public void onCreateFailure(String e){end("Ошибка соединения",true);}public void onSetFailure(String e){end("Ошибка соединения",true);}
  };}
  SdpObserver observer(Runnable success){return new SdpObserver(){public void onSetSuccess(){execute(success);}public void onSetFailure(String s){end("Ошибка соединения",true);}public void onCreateSuccess(SessionDescription s){}public void onCreateFailure(String s){end("Ошибка соединения",true);}};}
  void sendSdp(){if(sent||pc==null||pc.getLocalDescription()==null)return;String sdp=pc.getLocalDescription().description;if(sdp.length()>14000){end("Не удалось согласовать соединение",true);return;}sent=true;send(incoming?"call_answer":"call_offer",sdp);state=incoming?"Соединяем…":"Ожидаем ответа…";ChatService.changed(c);}
  void answer(String sdp){if(pc==null||sdp.length()>14000||pc.getRemoteDescription()!=null)return;pc.setRemoteDescription(observer(()->{state="Соединяем…";ChatService.changed(c);}),new SessionDescription(SessionDescription.Type.ANSWER,sdp));}
  ...
  void incomingNotice(){
   NotificationManager nm=c.getSystemService(NotificationManager.class);NotificationChannel ch=new NotificationChannel("incoming_calls","Входящие звонки",NotificationManager.IMPORTANCE_HIGH);
   ...
   nm.notify(410,new Notification.Builder(c,"incoming_calls").setSmallIcon(R.drawable.ic_launcher).setContentTitle("Звонок от @"+peer).setContentText("Голосовой звонок OldЫ Chat").setCategory(Notification.CATEGORY_CALL).setOngoing(true).setContentIntent(view).addAction(new Notification.Action.Builder(null,"Ответить",accept).build()).addAction(new Notification.Action.Builder(null,"Отклонить",reject).build()).build());ChatService.changed(c);
  }
