...
final class ProfileEditor {
 ...
 static void show(MainActivity a){try{
  ...
  section(a,b,I18n.t("ИДЕНТИФИКАТОР"));TextView handle=a.button("@"+owner.nick()+"   ⧉",false,()->{a.getSystemService(ClipboardManager.class).setPrimaryClip(ClipData.newPlainText(I18n.t("Ник Oldy Chat"),"@"+owner.nick()));Toast.makeText(a,I18n.t("Идентификатор скопирован"),Toast.LENGTH_SHORT).show();});handle.setOnClickListener(v->{a.getSystemService(ClipboardManager.class).setPrimaryClip(ClipData.newPlainText("Oldy username","@"+owner.nick()));Toast.makeText(a,I18n.t("Идентификатор скопирован"),Toast.LENGTH_SHORT).show();});b.addView(handle);a.space(b,6);a.paragraph(b,I18n.t("По этому адресу тебя находят другие участники."));
