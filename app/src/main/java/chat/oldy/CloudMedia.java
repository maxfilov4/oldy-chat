...
final class CloudMedia {
 static void attach(MainActivity a,Uri uri,String mime,String name,String target,String caption)throws Exception{
  Vault vault=a.vault;String vid="";boolean queued=false;JSONObject body=MediaFiles.importFile(a,uri,mime,name);String key=body.getString("local");
  try{
   ...
   if(mime.startsWith("image/"))body.put("thumb",a.makeThumb(key));body.remove("local");if(a.vault!=vault)throw new IOException("Аккаунт изменился");a.prepareRoom(target);String mid=vault.queuePayload(target,body,key);queued=true;registerLater(a.api,vault,mid,target);
 ...
 static String upload(Api api,Vault vault,String target,byte[] bytes,String kind)throws Exception{
  JSONObject request=new JSONObject().put("kind",kind).put("name","Медиа.mp4").put("room",Conversation.community(target)?Conversation.room(target):"").put("recipient",Conversation.community(target)?"":target).put("thread",Conversation.post(target)).put("size",bytes.length);
 ...
 static void download(Context context,Vault vault,JSONObject message)throws Exception{
  ...
  byte[] encrypted;try{if(c.getResponseCode()!=200)throw new IOException("Вложение недоступно или удалено");try(InputStream in=c.getInputStream();ByteArrayOutputStream out=new ByteArrayOutputStream()){byte[] b=new byte[16384];int n;while((n=in.read(b))!=-1){if(out.size()+n>MediaFiles.MAX+16)throw new IOException("Неверный размер");out.write(b,0,n);}encrypted=out.toByteArray();}}finally{c.disconnect();}
  Cipher cipher=Cipher.getInstance("AES/GCM/NoPadding");cipher.init(Cipher.DECRYPT_MODE,new SecretKeySpec(Crypto.un64(message.getString("blob_key")),"AES"),new GCMParameterSpec(128,Crypto.un64(message.getString("blob_iv"))));byte[] plain=cipher.doFinal(encrypted);if(plain.length!=message.getLong("size"))throw new IOException("Размер не совпал");String mid=message.getString("id");
  try(MediaFiles.Writer w=new MediaFiles.Writer(context,mid)){for(int at=0;at<plain.length;at+=16384)w.put(Arrays.copyOfRange(plain,at,Math.min(plain.length,at+16384)));String hash=w.finish();if(!hash.equalsIgnoreCase(message.getString("sha256"))){MediaFiles.path(context,mid).delete();throw new IOException("Проверка вложения не пройдена");}vault.attachment(mid,mid);}finally{Arrays.fill(plain,(byte)0);}
 }
 ...
 static void round(MainActivity a,File file,String target)throws Exception{
  ...
  try{byte[] bytes=java.nio.file.Files.readAllBytes(file.toPath());if(bytes.length>33554432)throw new IOException("Кружок слишком большой");vid=upload(a.api,vault,target,bytes,"round");
   JSONObject p=new JSONObject().put("kind","file").put("round",true).put("mime","video/mp4").put("name","Видеокружок").put("size",bytes.length).put("sha256",Crypto.hex(java.security.MessageDigest.getInstance("SHA-256").digest(bytes))).put("cloud_video",vid).put("thumb",CreatorVideo.thumbnail(a,Uri.fromFile(file)));
   if(a.vault!=vault)throw new IOException("Аккаунт изменился");a.prepareRoom(target);String mid=vault.queuePayload(target,p,null);queued=true;registerLater(a.api,vault,mid,target);a.runOnUiThread(()->{if(a.vault==vault&&a.chat.equals(target))a.renderMessages();});
