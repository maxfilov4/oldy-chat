...
public class VideoInstrumentation extends Instrumentation {
 ...
 public void onStart(){Bundle result=new Bundle();VideoActivity[] video={null};try{
  ...
  runOnMainSync(()->{video[0].player.seekTo(4000);video[0].setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);});Thread.sleep(1300);check(video[0].player.getCurrentPosition()>=3500,"Seeking did not advance");check(video[0].getResources().getConfiguration().orientation==Configuration.ORIENTATION_LANDSCAPE,"Landscape did not apply");shot("12-player-landscape");
