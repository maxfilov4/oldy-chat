...
public class FeatureInstrumentation extends Instrumentation {
 ...
 public void onStart(){Bundle out=new Bundle();Rtc[] rtc=new Rtc[2];try{
  ...
  Notices.prefs(getTargetContext()).edit().putBoolean("notifications",true).putBoolean("sound",true).putBoolean("preview",false).commit();Notices.show(getTargetContext(),b,b.message(gid),au);NotificationManager nm=getTargetContext().getSystemService(NotificationManager.class);check(nm.getActiveNotifications().length>0,"No message notification");check(nm.getNotificationChannel(Notices.channelId(getTargetContext())).getSound()!=null,"Notification has no sound");
