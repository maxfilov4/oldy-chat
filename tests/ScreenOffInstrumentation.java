...
public class ScreenOffInstrumentation extends Instrumentation {
 ...
 public void onStart(){Bundle out=new Bundle();try{
  ...
  getTargetContext().startActivity(new Intent(getTargetContext(),MainActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK));Thread.sleep(2500);
  ...
  JSONObject delivered=api.call("/send",e,registered.getString("token"));if(!delivered.optBoolean("delivered")||!v.has(mid))throw new Exception("No delivery with screen off");
